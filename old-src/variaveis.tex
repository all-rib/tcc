...
\title{Estudos, aplicação e adaptação de métodos de monitoramento de apiários em T. angustula} % Título do trabalho

%\titleFA{TÍTULO DO TRABALHO: SUBTÍTULO (SE HOUVER) PRECEDIDO DE DOIS PONTOS}

\titleen{Studies, application and adaptation of beehive monitoring methods for T. angustula} % Título traduzido para inglês
\advisor{Prof. Dr. Gustavo Weber Dernardin} % Nome do orientador. Lembre-se de prefixar com "Prof. Dr.", "Profª. Drª.", "Prof. Me." ou "Profª. Me."}
% Se não houver corientador, comente a linha a baixo
\coadvisor{Prof. Dr. Dalcimar Casanova} % Nome do coorientador, caso exista. Caso não exista, comente a linha.
...
\address{Pato Branco}
\addressen{Campus Pato Branco, PR, Brazil}
\documenttype{Monografia}
\documenttypeen{Monograph}
\degreetype{Graduação}

\evalboardmember{Cesar Claure Torrico)
}{Doutorado}{Universidade Tecnológica Federal do Paraná}
\evalboardmember{Kathya Silvia Collazos Linares)
}{Doutorado}{Universidade Tecnológica Federal do Paraná}
...
%
%
\PalavraChaveA{Apicultura de precis\~ao}%% Palavra-chave A
\PalavraChaveB{Tetragonisca angustula}%% Palavra-chave B
\PalavraChaveC{Sensoriamento}%% Palavra-chave C
\PalavraChaveD{Monitoramento}%% Palavra-chave D
\PalavraChaveE{Sistemas embarcados}%% Palavra-chave E
...
\KeywordA{Precision apiculture}%% Keyword A
\KeywordB{Tetragonisca angustula}%% Angustula}%% Keyword B
\KeywordC{Sensoring}%% Palavra-cSensoring}%% Keyword C
\KeywordD{Monitoring}%% Palavra-cMonitoring}%% Keyword D
\KeywordE{Embedded systems}%% Pal{Keyword 5}%% Keyword E
