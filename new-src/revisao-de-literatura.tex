...
I2C e SPI
	...
	I2C, ou Inter-Integrated Circuit, é um barramento de comunicação serial, inventado pela Philips
Semiconductors em 1980, para comunicar diversos dispositivos de
maneira síncrona no esquema mestre-escravo.

	É utilizado em diversos dispositivos de sensoriamento devida à
simplicidade das conexões, envolvendo 2 linhas de transmissão (SDA, ou
Serial Data, e SCL ou Serial Clock), podento atingir velocidades de
até 5Mbit/s. 

	De maneira semelhante, SPI, ou Serial Peripheral Interface, também
é um tipo de barramento, mas considerado por alguns autores como sendo
um padrão-de-facto. SPI no entanto difere de I2C por sua capacidade
full-duplex em suas linhas.

	Diferententemente daquela, SPI possui 4 linhas de transmissão (CS,
ou Chip Select, MISO, ou Master-In/Slave-Out, e MOSI, ou
Master-Out/Slave-In, a SCK ou Serial-Clock).

	Comparativamente, SPI tem vantagens sobre I2C por não possuir
velocidade máxima de clock, o que em teoria permite velocidades
ilimitadas de transmissão, uma garantia maior contra iterferências.
No entanto, essa possui a necessidade de mais pinos, um a mais por
dispositivo conectado, e possui problemas com longas distâncias quando
comparado com RS-232, ou RS-485. 

	Todos os sensores utilizados no projeto fazem uso desses dois
protocolos individualmente, mas problemas na interação com a colméia
serão demonstrados adiante. Infelizmente, não foram encontrados dados
sobre a interação de nehum dos métodos de comunicação colméias.
