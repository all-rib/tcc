...
graças à sua resiliência à crosstalk.
O uso de conectores RJ45 não é possivel, uma vez que a exposição à
chuva ou umidade danifica e provoca curto entre os terminais;
Pensando na possibilidade de falhas semelhantes, como oxidação, foi
utilizado um soquete provisório para interligar terminais comuns entre
sí (todos os terminais SCL, todos os SDA, GND's e VCC's), afim de permitir a remoção destes quando necessário. Além disso,
para evitar a necessidade de abrir a caixa para desconectar algum
sensor, outro conector foi feito na extensão dos fios entre a colméia,
e o controlador.
