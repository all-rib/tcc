...
\begin{itemize}
	...
	comportamento térmico e sonoro da colônia. Sensores do tipo SHT30,
	BME280 e DS18B20 foram utilizados devido à sua precisão e
	confiabilidade. 
	...
	integrar conectividade Wi-Fi, suporte a cartão SD e capacidade de
	processamento suficiente para operar múltiplos sensores
	simultaneamente. Os dados são armazenados em formato texto,
	contendo identificador, carimbo de tempo e unidade de medida,
	garantindo rastreabilidade e simplicidade na análise posterior.

	\item \textbf{Camada de comunicação e energia:} a transmissão de
	dados é realizada via rede sem fio, enquanto a energia é fornecida
	por baterias LiPo de 7,4~V e 5000~mAh, reguladas para 5~V através
	do circuito interno da placa. Essa configuração permite operação
	autônoma por longos períodos, reduzindo a necessidade de
	intervenção externa.  \end{itemize}

A interligação entre os sensores é feita por meio de cabos CAT6, aproveitando sua boa blindagem e condutividade elétrica. O barramento I\textsuperscript{2}C foi adotado como principal meio de comunicação, possibilitando a conexão de múltiplos sensores do mesmo tipo em paralelo. Cada dispositivo é identificado por um endereço de 7 bits, sendo os seis mais significativos fixos e o bit menos significativo definido pelo estado lógico do pino \textit{SDO}. Dessa forma, é possível utilizar dois sensores idênticos no mesmo barramento, diferenciando-os pelos endereços \texttt{0x76} e \texttt{0x77}. 

Para casos em que o número de sensores excede os endereços disponíveis, empregou-se uma adaptação baseada no protocolo SPI, utilizando o sinal \textit{chip select} (\textit{CS}) para alternar dinamicamente o dispositivo ativo. Essa solução permite que múltiplos sensores compartilhem a mesma linha de dados, preservando a integridade da comunicação.

Fisicamente, o módulo sensor é encapsulado em um frasco de vidro vedado com resina epóxi, prevenindo a entrada de umidade e contato direto com os insetos. As passagens de cabos são seladas de forma que o material ceroso das abelhas possa se moldar naturalmente aos condutores, sem comprometer o isolamento. Essa característica é fundamental, pois reduz o estresse comportamental da colônia e aumenta a aceitação do sensor dentro da colmeia.

A Figura~\ref{fig:arquitetura} ilustra o diagrama geral da arquitetura do sistema, apresentando as relações entre sensores, microcontrolador, armazenamento e interface de comunicação.
...
Essa organização modular permite que o sistema seja facilmente expandido ou adaptado a novas condições de medição. Além disso, a separação das camadas facilita o diagnóstico de falhas e o desenvolvimento de melhorias, como a substituição de sensores ou a alteração de protocolos de comunicação, sem a necessidade de modificar toda a estrutura do sistema.
...
\section{Elaboração da arquitetura}

Para o processo de sensoriamento, se determinou: é necessário
...
Para gerenciamento, processamento e armazenamento das informacoes, a
placa LORA32 da LILYGO foi utilizada, devida ao seu hardware integrado
para uso de cartoes SD, o que permite a gravacao dos dados durante um
grande periodo, bem como sua funcionalidade bluetooth nativa.
...
Os primeiros testes foram conduzidos fora das colmeias, com o objetivo
de verificar o funcionamento dos sensores e a estabilidade do sistema.
O protótipo permaneceu em operação contínua por três dias sem
apresentar falhas significativas ([Talvez citar graficos aqui]), sendo
observados apenas comportamentos inconsistentes do sensor DHT11,
posteriormente substituído por dispositivos de maior precisão.

Durante esta etapa, o sistema foi composto por sensores SHT30, DS18B20
e BME280, conectados ao microcontrolador ESP32 LORA32 da LilyGO. A
escolha desse módulo se deve à sua capacidade de armazenamento em
cartão SD e à integração nativa de comunicação sem fio. A alimentação
foi fornecida por baterias LiPo de 7,4 V, reguladas internamente para
5 V, e o cabeamento empregado foi do tipo CAT6, visando reduzir
interferências elétricas no barramento I²C.
...
Durante a operação em campo, surgiram falhas recorrentes no barramento
I²C, causando travamentos e reinicializações automáticas do sistema.
Tais eventos geravam acionamentos constantes do \textit{watchdog}, o
que, embora evitasse travamentos permanentes, resultava em alto
consumo energético e drenagem prematura da bateria.
...
projetada para lidar com restrições de memória e falhas de
comunicação. Essa ferramenta emprega filas do tipo \textit{First-In,
First-Out} (FIFO) e técnicas de média móvel e compactação de dados,
reduzindo a perda de amostras durante períodos de instabilidade.
Quando a mídia de gravação está indisponível, os dados são retidos
temporariamente em memória até que a operação possa ser concluída de
forma segura.
